-- Ensure assignment submissions cannot be created or updated after the deadline.
-- Removes the legacy permissive policy and re-applies deadline checks at RLS, trigger, and storage layers.
...
-- Legacy policy allowed INSERT/UPDATE without any deadline check when left behind by partial migrations.
