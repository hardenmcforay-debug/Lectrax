    ...
    <MarketingPageEnter className="bg-gradient-to-b from-slate-50 via-white to-slate-50">
      ...
      <section className="py-16 sm:py-20">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <LandingStagger className="grid gap-6 md:grid-cols-3">
            {PARTNERSHIP_PACKAGES.map((pkg) => (
              <LandingStaggerItem key={pkg.id}>
                <button
                  type="button"
                  onClick={() => setSelectedPackage(pkg.id)}
                  className={cn(
                    "relative w-full rounded-2xl border bg-white p-8 text-left shadow-sm transition-all hover:shadow-md",
